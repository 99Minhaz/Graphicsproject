...
void drawMoon() {
    ...
    // কাস্তে আকৃতির জন্য আকাশের রং দিয়ে ঢেকে দাও
    float sr, sg, sb;
...
void drawRoad() {
    ...
    // রাস্তার মাঝের ড্যাশ লাইন — হলুদ
    if (t > 0.6f || t < 0.15f)
        glColor3f(0.9f, 0.8f, 0.2f); // রাতে উজ্জ্বল হলুদ
    else
        glColor3f(0.85f, 0.85f, 0.2f); // দিনে সাধারণ হলুদ

    // ৮টি ড্যাশ সারি বরাবর
    for (int i = 0; i < 8; i++) {
...
void drawStreetLights() {
    ...
    // বাল্ব — রাতে হলুদ, দিনে ধূসর
    if (timeOfDay > 0.6f || timeOfDay < 0.15f)
        glColor3f(1.0f, 1.0f, 0.4f);  // রাতে হলুদ
    else
        glColor3f(0.6f, 0.6f, 0.6f);  // দিনে ধূসর
...
for (int i = 0; i < 300; i++) {
    ...
    s.y           =  0.0f + (rand() % 160) / 100.0f; // শুধু আকাশে
